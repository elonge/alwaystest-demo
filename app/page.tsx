...
export default function Home() {
  ...
  const handleOpenVariations = (productId: string) => {
    setSelectedProductId(productId)
    setPreviewVariationId(null)
    setIsScanning(true)
    setPanelProductId(null)
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-6 p-6 text-center">
      <div className="relative flex h-24 w-24 items-center justify-center">
        <span className="absolute h-full w-full rounded-full border-4 border-always-primary/40 border-t-always-primary animate-spin" />
        <span className="absolute h-16 w-16 rounded-full border-4 border-always-primary/20 border-b-always-primary/70 animate-[spin_3s_linear_infinite]" />
        <div className="flex h-12 w-12 items-center justify-center rounded-full bg-always-primary/10 text-always-primary">
      ...
      <div className="flex items-center justify-center gap-2">
        {[0, 1, 2].map((index) => (
          <span
            key={index}
            className="h-3 w-3 rounded-full bg-always-primary/80 animate-bounce"
